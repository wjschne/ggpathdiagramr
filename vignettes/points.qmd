...
If the angle is numeric instead of an angle, it is assumed to be in degrees The advantage of using the angle function is that under the hood, the `sinpi`, `cospi`, and `tanpi` functions are used to get the rounding right at key locations. For example, compare the y coordinate at $pi$ radians vs. 180 degrees:

```{r}
point(distance = 1, angle = 180)
point(distance = 1, angle = angle(degree = 180))
```
