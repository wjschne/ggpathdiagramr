...
library(ggplot2)
library(dplyr)
knitr::opts_chunk$set(dev = "svg", fig.height = 8, fig.width = 8)
```
...
ggplot() +
  ...
  circle(
    center = point(),
    radius = rev(seq(0.4, 1, length.out = 4)),
    fill = my_shades,
    color = NA,
    linewidth = .25
  ) +
  ...
  purrr::map2(r, 
              list(degree(theta), gradian(theta), radian(theta), turn(theta)), 
              \(radius, ang) {
                as.geom(label(polar(theta = ang, r = radius - .1), 
                              label = ang, 
                              fill = my_shades[5 - match(radius, r)],
                              ),
